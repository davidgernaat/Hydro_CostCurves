...
eta_DP         = 0.8;             %Efficiency turbine
lifetime    = 40;              %Years
%syear       = (60*60*24*365);  %Seconds in year... 8760 hr/yr
...
for j=1:numel(npipe_DP)
    ...
    for c = 1:10
        ...
        Re = D_DP.*rho.*u./mu;                                   % Reynold number
    ...
    Pror = rho*g*(Hgross-hf).*eta_DP*Qdesign;            % Capacity based on calculated Q (W)
    Pror_end{j} = Pror;
    ...
    PenstockCost = (6*D_DP + 9.4)*1e3*ER*Hgross; % $ Surface Penstock ($)
