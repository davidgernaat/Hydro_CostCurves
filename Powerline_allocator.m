function [DisCost, PowerlineTypeOut] = Powerline_allocator(D,PW)

%% Powerline allocator
% Determines which powerlines to build. Two sources for cost: (1) veileder
% and (2) Ontario case study. The second has 2x higher cost, see part in the bottom that is commented out

% D = distance in km
% PW = power capacity in MW

% Discost in in NOK or in $, depending on Veileder or Ontario source
...
PowerlineTypeOut=PowerlineType(1);
...
%
% DisCost = PlineC(PowerlineType(1));
